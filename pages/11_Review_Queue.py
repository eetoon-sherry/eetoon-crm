...
def _item_label(item) -> str:
    ...
    score = f"{payload.get('score_grade', '')}{payload.get('score', '')}".strip()
    return f"#{item.get('id')} {item.get('title')} | {email} | {score}"


def _summary_rows(items):
    rows = []
    for item in items:
        payload = _payload(item)
...
if item_type[0] == "candidate_company":
    ready_items = [item for item in items if _has_email(item)]
    if ready_items:
...
st.markdown("---")
selected_item = st.selectbox("选择一条查看/处理", items, format_func=_item_label)
if selected_item:
    _render_item(selected_item)
